...
const saltRounds = 12;
const path = require('path'); // Make sure you have this at the top if not already present
...
app.post('/submitUser', async (req,res) => {
    ...
    const randomPoints = Math.floor(Math.random() * 451) + 50; // (max - min + 1) + min

    if (!userCollection) {
        console.error("userCollection is not defined. Ensure database is connected and collection is set.");
    ...
    await userCollection.insertOne({
        ...
        points: randomPoints, // Add random points here
        profilePic: '/images/default-avatar.png', // Add a default profile picture
        displayName: username // Add a display name
    });
    console.log("Inserted user with points:", randomPoints);
...
app.get('/group_after_join', (req, res) => {
    res.sendFile(path.join(__dirname, 'public', 'group_after_join.html'));
});
